...
def load_images(file):
    im = Image.open(file)
    h= 332#im.size[0]-im.size[0]%4
    w= 512#im.size[1]-im.size[1]%4
...
def save_images3(filepath, result_1, result_2 = None):
    result_1 = np.squeeze(result_1)
    result_2 = np.squeeze(result_2)
    if not result_3.any():
        if not result_2.any():
...
def Guassianblur(I,p,winSize=(40,40)):
    mean_I = cv2.Guassianblur(I, winSize)      # I的均值平滑
    return mean_I[:,:,np.newaxis]

def mainFilter(I,winSize=(5,5)):
    mean_I = cv2.blur(I, winSize)      # I的均值平滑
    return mean_I.reshape(I.shape)
def meanFilter(I,winSize=(7,7)):
    # print(I.shape)
    I_ = np.squeeze(I)
...
def gasuss_noise(image, mean=0, var=0.01):
    ...
    return out
def gasuss_noise2(image, mean=0, var=0.05):
    ''' 
        添加高斯噪声
        mean : 均值 
        var : 方差
